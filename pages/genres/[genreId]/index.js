...
function GenreBooks(props) {
  ...
  return (
    ...
    >
      ...
      >
        Available Books ({props.data.length}) on {props.genreData}
      </h1>
      <div className="flex flex-wrap justify-center">
        {props.data.map((item) => (
          <CardContainer key={item._id} className="inter-var mr-4">
            <CardBody
...
export async function getServerSideProps(context) {
  ...
  let genre;
  try {
      const response=await axios.get(`http://localhost:3000/api/genres/${genreId}/books`);
      if(response.data){
        
        allGenreBooks=response.data.data;
  ...
  if (!allGenreBooks.length) {
    return {
      notFound: true,
    };
  }
  genre=allGenreBooks[0].genreId.name;
